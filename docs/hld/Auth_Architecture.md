...
**Status:** specification. Nothing here is implemented. This document says *what* must
exist and *why it is shaped this way*; `implementation/Master_Plan.md` Goals 21 and 22 say
*when*, and the `Goal_21_Plan.md` / `Goal_22_Plan.md` written afterwards will say *what was
actually built*.
...
`user_key` is a filesystem-safe encoding of `user_id`, validated against a strict pattern
...
`token_version` ends it within 15 minutes, but a live WebSocket chat stream is not
re-checking a token on every frame. Goal 21 should decide whether streams re-verify
periodically or are closed on revocation.

**OQ-A4 — Rate limiting lives where?** Per-IP limits on `/auth/*` are named here; the rest
of the API has none, and the BFF row in `Technical_HLD.md` §3.1 has claimed "auth, rate
limiting" as its responsibility since the first draft with neither implemented.

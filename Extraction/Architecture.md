...
The AI acts as a router to determine the relationship. It also acts as the **De-Fragmenter**, recognizing when a new extraction should map to an existing historical node despite vocabulary differences:
1. **Instance Match:** `{"action": "MERGE", "confidence": 0.95}` - "This new thought is another occurrence of *Past Pattern 1*."
2. **Support/Weight:** `{"action": "REINFORCE", "confidence": 0.94}` - "This isn't an identical instance to merge, nor does it evolve the pattern. It serves as strong supporting evidence that reinforces the 'weight' of an existing node."
3. **Evolution (Delta):** `{"action": "EVOLVE", "confidence": 0.92}` - "The old belief was X, but today's entry updates it to Y."
4. **Novel Branch:** `{"action": "BRANCH", "confidence": 0.88}` - "None of the historical candidates match. This is a completely new pattern."

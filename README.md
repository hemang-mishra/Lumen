...
Lumen is a personal wisdom system that converts unstructured voice and text journal entries into a richly connected, versioned knowledge graph. It is designed to solve "Personal Knowledge Decay" by extracting structured observations from messy human experiences.
...
3. **Model Agnostic & Abstracted (Tiered Routing):** Standard content uses default providers; sensitive content (`CRITICAL`) uses high-security configured providers via an abstracted Protocol layer.
...
- **[Query/Intelligence_Surfaces.md](Query/Intelligence_Surfaces.md):** Top-level "Applications" built on the graph (Personal Laws, Debugger, Decision Simulator).

### 4. Security & Abstraction
- **[hld/LLM_Abstraction_Architecture.md](hld/LLM_Abstraction_Architecture.md):** The 3-tier LLM routing model using Python Protocols, configurable providers, output scrubbing, and DPDP compliance.